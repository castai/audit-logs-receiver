...
Default image used in chart, which is `us-docker.pkg.dev/castai-hub/library/audit-logs-receiver`, is build with configuration from `builder-config.yaml` file from this repository. If you want to you could build your own image with different extensions and exporters as described in previous sections, and next you can override `image.repository` and `image.tag` properties in Helm chart.
